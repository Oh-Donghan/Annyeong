...
export default function DailyPlan({
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      // 첫 페이지 데이터 설정
      setTasks(allTasks.slice(0, PAGE_SIZE));
      setLastTaskVisible(allTasks[PAGE_SIZE - 1]);
      setFirstTaskVisible(allTasks[0]);
      setIsNext(allTasks.length > PAGE_SIZE);
      setIsPrev(false);
    });
  ...
  const handlePrev = () => {
    ...
    const newTasks = allTasks.slice(
      Math.max(0, currentIndex - PAGE_SIZE),
      currentIndex
    );
    setTasks(newTasks);
    setLastTaskVisible(newTasks[newTasks.length - 1]);
    setFirstTaskVisible(newTasks[0]);
    setIsPrev(currentIndex - PAGE_SIZE > 0);
    setIsNext(true);
  };
